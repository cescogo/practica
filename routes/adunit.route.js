...
adUnitRoutes.route('/update/:id').post(function (req, res) {
    ...
    if (!adUnit)
      return next(new Error('Could not load Document'));
        ...
        adUnit.user = req.body.user_name;
        adUnit.password = req.body.password;
        adUnit.permition=req.body.permition;
        adUnit.bodega=req.body.bodega;
...
//obtain specific user 
adUnitRoutes.route('/get/:user_name').get(function (req, res) {
  console.log("ruta adunit: "+req.params.user_name);
  AdUnit.findOne({user_name:req.params.user_name},function (err, adUnits){
  if(err){
    console.log(err);
  }
  else {
    console.log("resultado busqueda: "+adUnits.user_name);
    res.json(adUnits);
  }
});
});
